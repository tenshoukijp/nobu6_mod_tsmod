...
// ”NEŒE“ú‚æ‚èŒ³†•¶š—ñ‚ğ“¾‚éBŒE“ú‚ğÈ—ª‚µ‚½ê‡‚ÍA12Œ31“ú(”N––‚Ì“ú)‚ÆŒ©‚È‚·B
// ——R‚Í“VãÄ‹L‚Å‚ÍA‚Æ‚ ‚é”N‚Ì“r’†‚ÅŒ³†‚ª•Ï‚í‚Á‚Ä‚¢‚éê‡AVŒ³†‚Ì•û‚ğí‚ÉÌ—p‚·‚é‚½‚ßB
NB6WAREKI getWarekiFromYMD(int iYear, int iMonth, int iDay) {
	...
	for ( int i=GENGO_NUM-1; i >= 0; i-- ) {
		if ( iYMD >= gengo_list[i].start_YMD ) {
			...
			int iStartGengoYear = gengo_list[i].start_YMD / 10000; // ŠJn”N‚ğ“¾‚éB
			result.iyear = (iYear-iStartGengoYear)+1; // ˜a—ï“à‚Å‚Ì”N‚ğ“ü‚ê‚éBŒ³”N‚Í‚O‚Æ‚¢‚¤‚±‚Æ

			// Œ³”N‚Í“Á•Ê‚ÈŒ¾‚¢•û
			if ( result.iyear == 1 ) {
				strcpy( result.kanjiyear, "Œ³" ); // ŠJn”N‚Í“Á•Ê‚ÉuŒ³”Nv‚Æ‚¢‚¤Œ¾‚¢•û‚ğ‚·‚éB
				return result;

			// ‚»‚êˆÈŠO‚ÍŒvZ
			} else {
				// Œ³”NˆÈŠO‚Í‚P‚Â‚¸‚Â•ÏŠ·‚ª•K—vB
				...
				strcpy( result.kanjiyear, szBuf ); // ”¼Šp”š‚Éã‘‚«
	...
	// ‚±‚±‚Ü‚Å—ˆ‚½‚ç”»’è‚Å‚«‚È‚©‚Á‚½‚à‚ÌB‰Šú’l“I‚È‚à‚Ì‚ğ•Ô‚·
