...
//-----------------------------------------------------------------------------
// リソース番号から読み込めるように、特別版として追加。
// Name: CWaveFile::Open()
// Desc: Opens a wave file for reading
//-----------------------------------------------------------------------------
HRESULT CWaveFile::Open( int iResID, WAVEFORMATEX* pwfx, DWORD dwFlags, HMODULE hModule )
{
	HRESULT hr;
	...
	if( m_dwFlags == WAVEFILE_READ )
	{
		...
		if( NULL == m_hmmio )
		{
			...
			// Loading it as a file failed, so try it as a resource
			if( NULL == ( hResInfo = FindResource(hModule, MAKEINTRESOURCE(iResID), "WAVE") ) )
				return DXTRACE_ERR( "FindResource", E_FAIL );
